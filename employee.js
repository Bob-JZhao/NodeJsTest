...
/* execute initialize() function  after 50 ms*/
setTimeout(function() {
	initialize();
}, 50);
...
function initialize() {
	/* load employee function  by employer_id , employee_d ,period_id txnTypes and include documents */
	function load(callback) {
		Alloy.Globals.te.dispatch('/console/GetEmployee', {
			...
			bankAccounts = result.accounts || [];  // if accounts is null filling with []
			documents = result.documents || [];   // if documents is null filling with []
			callback();				
		});	
	}
   ...
   /* display differnt layout by osname like iphone and Android */
	...
	load(function() {
		/* create payrollSettings view set data on it */
		...
		/* add change , getYtdTxn and getBankAccounts functions on payrollSettingsSection view */
		...
		/* create detailSection view  set data on it  */
		...
		for (var i=0; i < rulesConfig.length; i++) {
			...
			if (currentRules[rule.id]) {
		 		...
		 		/* add click function when click row the showRuleSettings function will be called */
				...
				/* add change function when swt is changed the changeRuleSetting function will be called */
...
function clickDeduction(e) {
	var ctlr = Alloy.createController('employee/allowances', {   /* this is a deduction view so maybe the 
																	controller path is wrong ,it should be like employee/deduction */
		employeeTxn: employeeTxn,
		employeeId: employee_id,
		nav: nav,
		isDeduction: true,
